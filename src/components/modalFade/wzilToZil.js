...
import { WZIL_TO_ZIL, CONVERTING, DECIMAL, AMOUNT_TO_CONVERT, CANCEL_TEXT, NETWORK } from '../../config';

import { numberWithCommas } from '../helpers';
import { wzilToZilTransition } from '../../walletOperations/wzilToZil';

class WzilToZil extends Component {
    constructor(props) {
        super(props);
        this.state = {
            isLoaded: false,
...
class Slide1 extends Component {
    ...
    convertWzilTransition = (callback) => async () => {
        this.setState({ loading: true });

        let result = await wzilToZilTransition(window.zilPay, this.state.amount);
        ...
        let loadingContent = (
            <div>
                <div className={classes.loadingBorrowPillar}>
                    <CircularProgress />
                    <div>
                        {CONVERTING} {numberWithCommas(this.state.amount, DECIMAL[this.props.vaultFactory])} {this.props.vaultFactory}
                    </div>
                </div>
            </div>
        );
        ...
        let successContent = (
            <div>
                <div className={classes.successModal}>
                    ...
                    {/* TO_DO_AKSHAY */}
                    <div>
                        Converted: {numberWithCommas(this.state.amount, DECIMAL[this.props.vaultFactory])} {this.props.vaultFactory}{' '}
                    </div>
                ...
                <Card style={releaseCollateralCardStyle}>
                    <div>
                        <div className={classes.header}>
                            <div className={classes.heading}>{WZIL_TO_ZIL}</div>
                                    ...
                                    <div>
                                        <FormControl variant="filled" fullWidth={true}>
                                            ...
                                            <OutlinedInput
                                                ...
                                                endAdornment={<InputAdornment position="end">{this.props.vaultFactory}</InputAdornment>}
                                            />
                                            <FormButton
                                                disabled={!this.state.inputOk || !this.state.amount}
                                                text={WZIL_TO_ZIL}
                                                clickFunction={this.convertWzilTransition(this.props.cancelCallback)}
                                            />
...
export { WzilToZil as WzilToZilFade };
